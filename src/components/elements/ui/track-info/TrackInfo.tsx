...
export function TrackInfo({ title, subTitle, image, track }: ITrackInfoProps) {
		...
		<div className="flex items-center gap-3">
			{track ? (
				...
				>
					...
					<img src={image} alt={title} className="w-12 h-12 rounded-full m-1.5" />
				</button>
			) : (
			...
			<div>
				...
				<div className="flex items-start">{subTitle}</div>
			</div>
		</div>
	);
}
